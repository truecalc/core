...
pub fn register_financial(registry: &mut Registry) {
    registry.register_eager("PMT",  pmt::pmt_fn,  FunctionMeta { category: "financial", signature: "PMT(rate, nper, pv, [fv], [type])",       description: "Periodic payment for a loan" });
    ...
    registry.register_eager("PV",   pv::pv_fn,    FunctionMeta { category: "financial", signature: "PV(rate, nper, pmt, [fv], [type])",       description: "Present value" });
    registry.register_eager("FV",   fv::fv_fn,    FunctionMeta { category: "financial", signature: "FV(rate, nper, pmt, [pv], [type])",       description: "Future value" });
    registry.register_eager("RATE", rate::rate_fn,FunctionMeta { category: "financial", signature: "RATE(nper, pmt, pv, [fv], [type], [guess])",       description: "Interest rate per period" });
    registry.register_eager("NPER", nper::nper_fn,FunctionMeta { category: "financial", signature: "NPER(rate, pmt, pv, [fv], [type])",       description: "Number of payment periods" });
