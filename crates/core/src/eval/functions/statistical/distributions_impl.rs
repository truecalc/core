...
mod tests {
    ...
    fn norm_s_dist_cdf_at_zero_is_half() {
        let result = norm_s_dist_fn(&[num(0.0), bool_val(true)]);
        match result {
            Value::Number(v) => assert!(approx_eq(v, 0.5), "expected ~0.5, got {v}"),
    ...
    fn norm_s_dist_pdf_at_zero() {
        let result = norm_s_dist_fn(&[num(0.0), bool_val(false)]);
        match result {
            Value::Number(v) => assert!(approx_eq(v, 0.3989), "expected ~0.3989, got {v}"),
            other => panic!("expected Number, got {other:?}"),
        }
    }
    ...
    fn norm_dist_standard_cdf_at_zero() {
        ...
        let result = norm_dist_fn(&[num(0.0), num(0.0), num(1.0), bool_val(true)]);
        match result {
            Value::Number(v) => assert!(approx_eq(v, 0.5), "expected ~0.5, got {v}"),
            other => panic!("expected Number, got {other:?}"),
        }
    ...
    fn norm_dist_cdf_at_one_sigma() {
        ...
        let result = norm_dist_fn(&[num(1.0), num(0.0), num(1.0), bool_val(true)]);
        match result {
            Value::Number(v) => assert!(approx_eq(v, 0.8413), "expected ~0.8413, got {v}"),
            other => panic!("expected Number, got {other:?}"),
        }
    }
    ...
    fn norm_dist_stdev_zero_is_num_error() {
        let result = norm_dist_fn(&[num(0.0), num(0.0), num(0.0), bool_val(true)]);
    ...
    fn norm_s_inv_at_half_is_zero() {
        let result = norm_s_inv_fn(&[num(0.5)]);
        match result {
            Value::Number(v) => assert!(approx_eq(v, 0.0), "expected ~0.0, got {v}"),
            other => panic!("expected Number, got {other:?}"),
        }
    }
    ...
    fn norm_s_inv_at_0975_is_1_96() {
        let result = norm_s_inv_fn(&[num(0.975)]);
        match result {
            Value::Number(v) => assert!(approx_eq(v, 1.96), "expected ~1.96, got {v}"),
            other => panic!("expected Number, got {other:?}"),
        }
    }
    ...
    fn standardize_basic() {
        // STANDARDIZE(2, 1, 1) → 1.0
        let result = standardize_fn(&[num(2.0), num(1.0), num(1.0)]);
        assert_eq!(result, Value::Number(1.0));
    }

    #[test]
    fn standardize_x_equals_mean_is_zero() {
        let result = standardize_fn(&[num(5.0), num(5.0), num(2.0)]);
        assert_eq!(result, Value::Number(0.0));
    }

    #[test]
    fn standardize_stdev_zero_is_num_error() {
        let result = standardize_fn(&[num(2.0), num(1.0), num(0.0)]);
        assert_eq!(result, Value::Error(ErrorKind::Num));
    }
    ...
    fn correl_perfect_positive() {
        let result = correl_fn(&[nums_array(&[1.0, 2.0, 3.0]), nums_array(&[1.0, 2.0, 3.0])]);
        match result {
            Value::Number(v) => assert!(approx_eq(v, 1.0), "expected ~1.0, got {v}"),
            other => panic!("expected Number, got {other:?}"),
        }
    }
    ...
    fn correl_perfect_negative() {
        let result = correl_fn(&[nums_array(&[1.0, 2.0, 3.0]), nums_array(&[3.0, 2.0, 1.0])]);
        match result {
            Value::Number(v) => assert!(approx_eq(v, -1.0), "expected ~-1.0, got {v}"),
            other => panic!("expected Number, got {other:?}"),
        }
    ...
    fn pearson_perfect_positive() {
        let result = pearson_fn(&[nums_array(&[1.0, 2.0, 3.0]), nums_array(&[1.0, 2.0, 3.0])]);
        match result {
            Value::Number(v) => assert!(approx_eq(v, 1.0), "expected ~1.0, got {v}"),
            other => panic!("expected Number, got {other:?}"),
        }
    ...
    fn poisson_pmf_at_zero_lambda_1() {
        ...
        let result = poisson_fn(&[num(0.0), num(1.0), bool_val(false)]);
        match result {
            Value::Number(v) => assert!(approx_eq(v, 0.3679), "expected ~0.3679, got {v}"),
            other => panic!("expected Number, got {other:?}"),
        }
    }
    ...
    fn poisson_cdf_at_zero_lambda_1() {
        // POISSON(0, 1, true) ≈ e^-1 ≈ 0.3679
        let result = poisson_fn(&[num(0.0), num(1.0), bool_val(true)]);
        match result {
            Value::Number(v) => assert!(approx_eq(v, 0.3679), "expected ~0.3679, got {v}"),
            other => panic!("expected Number, got {other:?}"),
        }
    ...
    fn norm_inv_at_half_is_mean() {
        // NORM.INV(0.5, 3, 1) ≈ 3.0 (median = mean for normal distribution)
        let result = norm_inv_fn(&[num(0.5), num(3.0), num(1.0)]);
        match result {
            Value::Number(v) => assert!(approx_eq(v, 3.0), "expected ~3.0, got {v}"),
            other => panic!("expected Number, got {other:?}"),
        }
    }
    ...
    fn norm_inv_stdev_zero_is_num_error() {
        let result = norm_inv_fn(&[num(0.5), num(0.0), num(0.0)]);
        assert_eq!(result, Value::Error(ErrorKind::Num));
    }
    ...
    fn expon_dist_cdf_at_zero_is_zero() {
        // EXPON.DIST(0, 1, true) = 1 - e^0 = 0
        let result = expon_dist_fn(&[num(0.0), num(1.0), bool_val(true)]);
        assert_eq!(result, Value::Number(0.0));
    }

    #[test]
    fn expon_dist_pdf_at_zero_equals_lambda() {
        // EXPON.DIST(0, 2, false) = 2 * e^0 = 2
        let result = expon_dist_fn(&[num(0.0), num(2.0), bool_val(false)]);
        assert_eq!(result, Value::Number(2.0));
    }

    #[test]
    fn expon_dist_lambda_zero_is_num_error() {
        let result = expon_dist_fn(&[num(1.0), num(0.0), bool_val(true)]);
        assert_eq!(result, Value::Error(ErrorKind::Num));
    }
    ...
    fn binom_dist_pmf_certain_success() {
        // BINOM.DIST(1, 1, 1.0, false) = 1.0 (k=n, p=1)
        let result = binom_dist_fn(&[num(1.0), num(1.0), num(1.0), bool_val(false)]);
        assert_eq!(result, Value::Number(1.0));
    }

    #[test]
    fn binom_dist_cdf_all_successes() {
        // BINOM.DIST(3, 3, 0.5, true) = 1.0 (CDF at max k = n)
        let result = binom_dist_fn(&[num(3.0), num(3.0), num(0.5), bool_val(true)]);
        assert_eq!(result, Value::Number(1.0));
    }

    #[test]
    fn binom_dist_k_gt_n_is_num_error() {
        let result = binom_dist_fn(&[num(4.0), num(3.0), num(0.5), bool_val(false)]);
        assert_eq!(result, Value::Error(ErrorKind::Num));
    }
    ...
    fn fisher_inv_at_zero_is_zero() {
        let result = fisher_inv_fn(&[num(0.0)]);
        assert_eq!(result, Value::Number(0.0));
    }
    ...
    fn fisher_roundtrip() {
        ...
        let z = fisher_fn(&[num(0.5)]);
        let x = match z {
            Value::Number(v) => fisher_inv_fn(&[num(v)]),
    ...
    fn gauss_at_zero_is_zero() {
        // GAUSS(0) = norm_s_cdf(0) - 0.5 ≈ 0
        let result = gauss_fn(&[num(0.0)]);
        match result {
            Value::Number(v) => assert!(approx_eq(v, 0.0), "expected ~0.0, got {v}"),
            other => panic!("expected Number, got {other:?}"),
        }
    ...
    fn covar_identical_arrays() {
        // COVAR([1,2,3], [1,2,3]) = variance = 2/3
        let result = covar_fn(&[nums_array(&[1.0, 2.0, 3.0]), nums_array(&[1.0, 2.0, 3.0])]);
        match result {
            Value::Number(v) => assert!(approx_eq(v, 2.0 / 3.0), "expected ~0.6667, got {v}"),
            other => panic!("expected Number, got {other:?}"),
        }
    }
